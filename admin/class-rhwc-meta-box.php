...
class RHWC_Meta_Box {
	...
	public function add_meta_box() {
		add_meta_box(
			...
			__( 'Generate Reviews (Review Helper)', 'rhwc' ),
	...
	public function render_meta_box( $post ) {
		...
		<div class="rhwc-meta-box-wrapper">
			<p>
				<label for="rhwc_count"><strong><?php esc_html_e( 'Number of Reviews:', 'rhwc' ); ?></strong></label>
				<input type="number" id="rhwc_count" class="widefat" value="<?php echo esc_attr( $default_count ); ?>" min="1" max="100">
			</p>
			<p>
				<label for="rhwc_rating"><strong><?php esc_html_e( 'Rating Range:', 'rhwc' ); ?></strong></label>
				<select id="rhwc_rating" class="widefat">
					<option value="5" <?php selected($default_rating, '5'); ?>>5 Stars</option>
					<option value="4-5" <?php selected($default_rating, '4-5'); ?>>4-5 Stars</option>
					<option value="3-5" <?php selected($default_rating, '3-5'); ?>>3-5 Stars</option>
					<option value="1-5" <?php selected($default_rating, '1-5'); ?>>1-5 Stars</option>
				</select>
			</p>
			<p>
				<label for="rhwc_names"><strong><?php esc_html_e( 'Reviewer Names (One per line):', 'rhwc' ); ?></strong></label>
			...
			<p>
				<button type="button" class="button button-primary" id="rhwc-generate-btn" data-product-id="<?php echo esc_attr( $post->ID ); ?>">
					<?php esc_html_e( 'Generate Reviews', 'rhwc' ); ?>
				</button>
