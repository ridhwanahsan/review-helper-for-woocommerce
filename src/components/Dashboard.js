...
const Dashboard = ( { setActiveTab, globalData } ) => {
    ...
    const quickActions = [
        bulkEnabled && {
            key: 'bulk',
            title: 'Bulk Generate',
            subtitle: 'Create review batches for multiple products from one clean workflow.',
            tag: 'Fastest',
            emphasis: true,
            icon: (
        ...
        },
        {
            ...
            subtitle: 'Approve, edit, or remove sample reviews directly from the review table.',
            tag: 'Review',
            icon: (
                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={ 2 }>
        ...
        },
        {
            key: 'settings',
            title: 'Settings',
            subtitle: 'Control defaults, reviewer names, custom comments, and plugin safeguards.',
            tag: 'Setup',
            icon: (
                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={ 2 }>
    ...
    const overviewItems = [
        {
            ...
            value: customCommentsEnabled ? 'Custom comment library' : contentStyle,
        },
        {
            label: 'Default reviewer names',
            value: customNamesEnabled ? 'Custom list added' : 'Using built-in names',
        },
        {
        ...
        <div className="rhwc-page-shell">
            <div className="rhwc-page-hero rhwc-page-hero--dashboard">
                <div className="rhwc-page-hero__main">
                    <span className="rhwc-page-hero__eyebrow">WooCommerce dashboard</span>
                    <h2 className="rhwc-page-hero__title">Review Helper workspace</h2>
                    <p className="rhwc-page-hero__description">
                        Monitor review activity, jump into the right workflow quickly, and keep your saved generation defaults visible from one place.
                    </p>
                    ...
                    <div className="rhwc-page-hero__chips">
                        ...
                        <span className="rhwc-page-hero__chip">{ bulkEnabled ? 'Bulk tools enabled' : 'Bulk tools hidden' }</span>
                    </div>
                </div>

                <div className="rhwc-page-hero__stats">
                    <div className="rhwc-page-stat">
                        <span className="rhwc-page-stat__label">Last generated</span>
                        <strong className="rhwc-page-stat__value">{ lastGeneratedDate }</strong>
                    </div>
                    <div className="rhwc-page-stat">
                        <span className="rhwc-page-stat__label">Default rating</span>
                        <strong className="rhwc-page-stat__value">{ formatRatingLabel( defaultRating ) }</strong>
                    </div>
                </div>
            </div>
            ...
            <div className="rhwc-dashboard-highlights">
                { highlightCards.map( ( card ) => (
                    <div key={ card.label } className="rhwc-dashboard-highlight">
                        <span className="rhwc-dashboard-highlight__label">{ card.label }</span>
                        <strong className="rhwc-dashboard-highlight__value">{ card.value }</strong>
                        <span className="rhwc-dashboard-highlight__meta">{ card.meta }</span>
                    </div>
                ) ) }
            </div>
            ...
            <div className="rhwc-page-grid rhwc-page-grid--sidebar">
                <div className="rhwc-dashboard-stack">
                    <div className="rhwc-card rhwc-card--soft">
                        <div className="rhwc-card-header">
                            <div>
                                <h2>Quick actions</h2>
                                <p>Open the most-used sections instantly without searching through the sidebar.</p>
                            </div>
                            <span className="rhwc-card-badge">Daily workflow</span>
                        </div>
                        ...
                        <div className="rhwc-actions-grid">
                            { quickActions.map( ( action ) => (
                                <button
                                    key={ action.key }
                                    type="button"
                                    className={ `rhwc-action-btn ${ action.emphasis ? 'rhwc-action-btn--primary' : '' }` }
                                    onClick={ () => setActiveTab( action.key ) }
                                >
                                    <div className="rhwc-action-btn__icon">{ action.icon }</div>

                                    <div className="rhwc-action-btn__content">
                                        <div className="rhwc-action-btn__topline">
                                            <span className="rhwc-action-btn__title">{ action.title }</span>
                                            <span className="rhwc-action-btn__tag">{ action.tag }</span>
                                        </div>
                                        <span className="rhwc-action-btn__sub">{ action.subtitle }</span>
                                    </div>
                    ...
                    <div className="rhwc-card rhwc-card--soft">
                        <div className="rhwc-card-header">
                            <div>
                                ...
                                <p>A quick explanation of how the generator is currently set up and what to review next.</p>
                            </div>
                ...
                <aside className="rhwc-card rhwc-card--soft">
                    <div className="rhwc-card-header">
                        <div>
                            <h2>Current setup</h2>
                            <p>Your saved defaults at a glance, so it is easy to spot what is configured right now.</p>
                        </div>
                    </div>
