...
const App = () => {
    ...
    const visibleNavItems = NAV_ITEMS.filter( ( item ) => ! item.requireBulk || enableBulk );
    ...
    return (
        <div className={ `rhwc-app ${ sidebarCollapsed ? 'rhwc-app--collapsed' : '' }` }>
                ...
                <nav className="rhwc-sidebar__nav">
                    { visibleNavItems.map( ( item ) => (
                        <button
                            key={ item.id }
                            type="button"
                            className={ `rhwc-nav-item ${ activeTab === item.id ? 'rhwc-nav-item--active' : '' }` }
                            onClick={ () => setActiveTab( item.id ) }
                            title={ sidebarCollapsed ? item.label : '' }
                        >
                            <span className="rhwc-nav-item__icon-wrap">
                                <span className="rhwc-nav-item__icon">{ item.icon }</span>
                            </span>

                            { ! sidebarCollapsed && (
                                <>
                                    <span className="rhwc-nav-item__content">
                                        <span className="rhwc-nav-item__label">{ item.label }</span>
                                        <span className="rhwc-nav-item__meta">{ item.description }</span>
                                    </span>

                                    <span className="rhwc-nav-item__chevron" aria-hidden="true">
                            ...
                            { activeTab === item.id && <span className="rhwc-nav-item__indicator" /> }
                        </button>
                    ) ) }
                </nav>
                ...
                { ! sidebarCollapsed && (
                    <div className="rhwc-sidebar__footer">
                        <div className="rhwc-sidebar__footer-card">
                            <span className="rhwc-sidebar__footer-label">Plugin status</span>
                            <strong>Ready to generate</strong>
                            <div className="rhwc-sidebar__footer-meta">
                                <span>{ enableBulk ? 'Bulk enabled' : 'Bulk hidden' }</span>
                                <span>{ `${ totalGenerated } total reviews` }</span>
                            </div>
                        </div>

                        <span className="rhwc-version-badge">v1.0.0</span>
                    </div>
                ) }
            </aside>
                ...
                <div className="rhwc-topbar">
                    <div className="rhwc-topbar__heading">
                        <div className="rhwc-topbar__breadcrumb">
                            <span className="rhwc-topbar__plugin">Review Helper</span>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={ { opacity: 0.4 } }>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                            </svg>
                            <span className="rhwc-topbar__page">{ PAGE_TITLES[ activeTab ] }</span>
                        </div>
                        <p className="rhwc-topbar__description">{ PAGE_DESCRIPTIONS[ activeTab ] }</p>
                    </div>
